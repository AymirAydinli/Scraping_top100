...
gecko_path = '/usr/local/bin/chromedriver 2'  # location of the driver is declared
...
d = pd.DataFrame({'Title':[], 'Tatometer':[], 'Audience Score':[], 'MPAA':[], 'Genre':[], 'Director':[], 'Writer':[],'Box Office':[] }) # dataframe is made using pandas


shows=driver.find_elements_by_xpath("//td/a[@class='unstyled articleLink']")  # all the <a> tags of the shows is stored as a list
...
#try except helps to avoid crashes or errors if xpath is not found
    ...
    try:
        ...
        audience  = driver.find_element_by_xpath("//div[@class='mop-ratings-wrap__half audience-score']/h2/a/span[@class='mop-ratings-wrap__percentage']").text
        print(audience)
    except:
        print("Not Found")
